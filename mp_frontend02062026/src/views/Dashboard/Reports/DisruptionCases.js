...
function DisruptionCases() {
  ...
  const getTableConfig = () => [
    ...
    { key: "lastName", render: (item) => item.lastName },
    {
      ...
      render: (item) => {
        if (item.HTChildCurrentPlacementStatusId && childCurrentPlacementList) {
          const placementStatus = childCurrentPlacementList.find(
            (child) => child.id === item.HTChildCurrentPlacementStatusId
          );
          return placementStatus
            ? placementStatus.currentPlacementStatus
            : "--";
        }
        return "--";
      },
    ...
    { key: "lastFollowUpDate", render: (item) => item.lastFollowUpDate },
    {
  ...
  useEffect(() => {
    document.title = "Reports | Disruption Cases | ThriveWell";
    if (initialData === null) {
      getDisruptionCases();
    } else if (initialData) {
      let payload = {
      ...
      await APIS.DisruptionCases(finalPayload).then((resp) => {
        if (resp && resp.data) {
          ...
          if (
            ...
            resp.data?.message?.data.length === 0
          ) {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const data = await APIS.DisruptionCases(finalPayload);
      if (data?.data) {
        ConvertToXLSX(data?.data, "Disruption Cases");
        setIsExportDisabled(false);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
      ...
      >
        <ReportHeader reportHeaderText={t("common:common.DisruptionCases")} />
        <ReportExportButton
