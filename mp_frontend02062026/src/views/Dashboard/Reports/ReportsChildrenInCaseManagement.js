...
const tableBody = [
  { key: "childId" },
  { key: "childPlacementStatus" },
  { key: "dateofLastThriveScale" },
];

function ReportsChildrenInCaseManagement() {
  ...
  let tableHead = [
    ...
    { name: t("common:common.Date of Last Thrive Scale") },
  ];
  ...
  useEffect(() => {
    document.title = "Reports | Children in Case Management | ThriveWell";
    if (initialData === null) {
      getNewlyAddedChildrenApi();
    } else if (initialData) {
      let payload = {
  ...
  const getNewlyAddedChildrenApi = useCallback(async (payload = null) => {
    ...
    try {
      ...
      await APIS.CaseManagementReport(finalPayload).then((resp) => {
        if (resp && resp.data) {
  ...
  return (
    ...
    >
      <ReportHeader
        reportHeaderText={t("common:common.Children in Case Management")}
      />
      {/* <ReportExportButton
...
export default ReportsChildrenInCaseManagement;
