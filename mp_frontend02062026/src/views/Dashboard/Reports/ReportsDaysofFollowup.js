...
const tableBody = [
  { key: "familyId" },
  { key: "firstPostReintegrationDate" },
  { key: "closedDate" },
...
function ReportsDaysofFollowup() {
  ...
  useEffect(() => {
    document.title = "Reports | Days of Followup | ThriveWell";
    if (initialData === null) {
      getDaysofFollowupApi();
    } else if (initialData) {
      let payload = {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const data = await APIS.DurationFollowupReport(finalPayload);
      if (data?.data) {
        ConvertToXLSX(data?.data, "Days of follow-up");
        setIsExportDisabled(false);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    ...
    >
      <ReportHeader reportHeaderText={t("common:common.Days of Followup")} />
      <ReportExportButton
