...
const IncrisisAndVulnerableMilestonesChildren = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await APIS.GetIncrisisAndVulnerableMilestonesChildren(
        payload
      );
      const incrisisAndVulnerableMilestonesChildren = response.data.data || [];
      const transformedData = incrisisAndVulnerableMilestonesChildren.map(
        (item, idx) => ({
  ...
  return (
    <MultiLineGraph
      title="In crisis and Vulnerable milestones for all children, by assessment"
      data={chartData}
...
export default IncrisisAndVulnerableMilestonesChildren;
