...
import { Box, Button, Card, CircularProgress, Divider, Grid, TextField, Typography, useTheme } from '@material-ui/core';
...
const AddChildForm = (props) => {
  ...
  const { organizationList,
    ...
    getChildFamilyList,
    signedinOrgType,
    signedinUserRole,userRegion } = useContext(CommonDataContext);
  const { fromFam, famNumber } = props;
  let savedOrgid;
    ...
    if(values.organization_name !== ''){
      ...
      try {
        //values.country ='1'
         ...
         setCciData(orgData)
         setFieldValue('country',orgData.HTCountryId)
         setFieldValue('state',orgData.HTStateId)
         setFieldValue('district',orgData.HTDistrictId)
  ...
  useEffect(() => {
    console.log(`%c${value}`,"display:none")
    ...
    if((signedinOrgType == 3 || signedinOrgType == 4 || signedinOrgType == 5) && (signedinUserRole === 'admin' || signedinUserRole === 'caseworker')){
      // has access
      if(currentOrganization !== signedinOrgId && signedinOrgType !== 5){
  ...
  return (
    <Formik
      ...
      initialValues={{
        ...
        addDate:null,
        closedDate:null,
        organization_name:signedinOrgType !== 5?signedinOrgId : '',
        genderOption: '',
        gender:'',
        phone:'',
        email:'',
        language:'',
        submit: null,
        family:fromFam ? famNumber : '',
        childPlacement:'',
        childStatus:'',
        childEducation:'',
        childEducationSpecify:'',
        childCurrentPlacement:'',
        caseWorker:'',
      }}
      validationSchema={Yup
        ...
        .shape({
          ...
          birthdate: Yup.string().max(255).required(t('common:warnings.Date of Birth is required')).nullable(),
          //addDate: Yup.string().max(255),
          ...
          organization_name: Yup.string().max(255).required(t('common:warnings.Organization Name is required')),
      ...
      onSubmit={async (values, { resetForm, setErrors, setStatus, setSubmitting }) => {
        let payload = {
          ...
          "email": values.email,
          "dateOfEntry": values.addDate !== ''  && values.addDate !== null?getDate(values.addDate):null, 
          "dateOfExit" : values.closedDate !== ''  && values.closedDate !== null ? getDate(values.closedDate) : null,
    ...
    try { 
      await APIS.AddChild(payload).then((res)=>{
        if(res && res.data && (res.status === 200 || res.status === 201)){
          ...
          savedOrgid = res.data.childId;
          addCase(res.data.childId,values.caseWorker)
    ...
    >
      {({ errors, handleBlur, handleChange, handleSubmit, handleReset,isSubmitting, touched, values, setFieldValue, setFieldError }) => (
        <Form
        ...
        >
          ...
          />
            ...
            component={AutoCompleteDropdown}
            getOrgDetails={()=> getOrgDetails(values,setFieldValue)}
            required={true}
          ...
          />
                ...
                >
                  <TextField
                    ...
                    onBlur={handleBlur}
                    onChange={handleChange}
                    value={cciData?.city?cciData?.city:values.city}
                    variant="outlined"
                  />
                </Grid>
                <Grid
                ...
                >
                  {/* <TextField
                    ...
                    value={cciData?.zipCode?cciData?.zipCode:values.zip_code}
                    variant="outlined"
                  /> */}
                  <NumberFormat
                    customInput={TextField}
                ...
                >
                  <TextField
                    ...
                    onBlur={handleBlur}
                    onChange={handleChange}
                    value={cciData?.addressLine1?cciData?.addressLine1:values.address1}
                    variant="outlined"
                  />
                </Grid>
                <Grid
                ...
                >
                  <TextField
                    ...
                    onBlur={handleBlur}
                    onChange={handleChange}
                    value={cciData?.addressLine2?cciData?.addressLine2:values.address2}
                    variant="outlined"
                  />
                </Grid>
                {/* <Grid
              ...
              <Box sx={{ mt: 2,display : "flex",flexDirection : "row" }}>
                ...
                <Button
                  ...
                  onClick={()=>{handleReset();
                    ...
                    values.addDate=new Date();
                    values.closedDate=null}}
