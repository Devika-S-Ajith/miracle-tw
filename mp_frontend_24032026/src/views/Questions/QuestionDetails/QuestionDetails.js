...
const QuestionDetails = () => {
  ...
  const {signedinUserRole,signedinOrgType, languageList} = useContext(CommonDataContext);
  const currentLanguage = localStorage.getItem('language');
  const getLanguageId = () => {
    const langId = languageList.length && languageList.find(item => item.languageCode == currentLanguage)?.id
    return langId;
  }
  const [selectedLanguageId, setSelectedLanguageId] = useState(currentLanguage === 'en' ? null : getLanguageId());
  const [question, setQuestion] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  const [currentTab, setCurrentTab] = useState('details');
  const [options] = useState([{ "choiceName": "In-crisis", "score": "1" },
  ...
  const getQuestionDetails = useCallback(async (languageId) => {
    document.title = "Questions | Details | Miracle Foundation"
    setIsLoading(true)
    try {
  ...
  useEffect(() => {
    const languageId = currentLanguage === 'en' ? null : getLanguageId()
