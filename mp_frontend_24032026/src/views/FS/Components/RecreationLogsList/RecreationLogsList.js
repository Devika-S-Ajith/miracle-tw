...
const RecreationLogsList = ({ module, listData }) => {
  const { id } = useParams();
  const [recLog, setRecLog] = useState(null);
  ...
  const recId = searchParams && searchParams.get("recreationlog");
  ...
  const getRecLogList = useCallback(
    async (rowCount, page, fromDate, toDate, query) => {
      ...
      try {
        ...
        setRecLog(data && data.data && data.data.data && data.data.data?.logs);
  ...
  const handleExport = useCallback(async () => {
    try {
      ...
      if (data && data?.data) {
        PrintAsPDF(data?.data, "RECREATION LOG");
  ...
  return (
    <Box>
      ...
      <Typography
        ...
        fontWeight={700}
      >
        Recently submitted recreation logs
      </Typography>
      ...
      <Box mt={2} sx={{ overflow: "auto" }}>
        {recLog && recLog.length > 0 ? (
          <Table>
            <TableHead>
              <TableRow>
                {(module !== "children") && <TableCell>Child</TableCell>}
                <TableCell>Date</TableCell>
                <TableCell>Family activity</TableCell>
                <TableCell>Activity comment</TableCell>
                <TableCell></TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {recLog.map((RecLogListItem) => {
                return (
                  <TableRow hover key={RecLogListItem.id}>
                    ...
                    <TableCell>
                      {Array.isArray(
                        RecLogListItem?.formAnswer?.formResponse[2]
                          .components[3]?.responseValue
                      )
                        ? RecLogListItem?.formAnswer?.formResponse[2].components[3]?.responseValue
                            .filter((item) => item !== "")
                            .map((item) => item)
                            .join(", ")
                        : RecLogListItem?.formAnswer?.formResponse[2]
                    ...
                    <TableCell>
                      {
                        RecLogListItem?.formAnswer?.formResponse[1]
                          .components[0]?.responseValue
                      }
                    </TableCell>
                    ...
                    <TableCell>
                      <RemoveRedEyeIcon
                        ...
                        onClick={() => {
                          navigate(
                            `/fosterShare/${module}/${id}/?recreationlog=${RecLogListItem.id}`
                          );
                        }}
                      />
                    </TableCell>
...
export default RecreationLogsList;
