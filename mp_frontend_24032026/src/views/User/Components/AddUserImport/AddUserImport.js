	...
	const getHeaderValue = useCallback(async (id) => {
		try {
			...
			if (data) {
				...
				setUserHeader(data.data.entityProperties);
				setLoading(false)
					...
					<Box>
						...
						>
							<Table>
								...
								<TableBody sx={{ mb: 4 }}>
									<TableRow sx={{ mb: 4 }}>
										<TableCell sx={{ fontSize: '12pt', fontWeight: "bold", ml: 4 }}>
											<TextField inputProps={{ style: { fontSize: '12pt', fontWeight: "bold" } }} InputProps={{ disableUnderline: true }} value="User Field" variant="standard" />
								...
								{userHeader && userHeader.length > 0 && userHeader.map((item, index) => (
									<TableBody sx={{ mt: 4 }}>
										{item.isMandatory ?
