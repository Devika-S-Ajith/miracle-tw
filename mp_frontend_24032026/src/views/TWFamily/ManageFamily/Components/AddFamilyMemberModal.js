...
const AddFamilyMemberModal = ({ onClose, getMemberDetails,familyId, member, isFamilyActive = true, isMemberActive = true, dropdownValues = {} }) => {
    ...
    // valuesRef is updated every render via assignment in render props — no setState, no loop
    ...
    // Stable handler — reads refs, never recreated, won't cause re-render loops
    const handleCancelClick = useCallback(() => {
        onClose();
        return;
        if (!isDirty(valuesRef.current, initialValuesRef.current)) {
            onClose();
            return;
        }
            ...
            onSubmit={async (values, { setErrors, setStatus, setSubmitting }) => {
                ...
                const payload = {
                    ...
                    ...(['phoneNumber', 'occupation', 'note', 'appAccessEnabled', 'email'].some(k => changed[k] !== undefined) && {
                        profileInformation: {
                            ...(changed.phoneNumber !== undefined  && { phoneNumber: changed.phoneNumber }),
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            >
                                ...
                                <Button
                                    ...
                                    sx={{ visibility: !(isMemberActive) ? "hidden" : "visible" }}
                                    onClick={() => {
                                            ...
                                            <LocalizationProvider dateAdapter={AdapterDayjs}>
                                                <Box sx={{ p: 3 }}>
                                                    ...
                                                    <Box sx={{ mb: 2 }}>
                                                        ...
                                                        <DatePicker
                                                            value={values.memberClosureDate ? dayjs(values.memberClosureDate) : null}
                                                            format={DateFormatFromRegion(true)}
                                                            onChange={(newValue) => setFieldValue('memberClosureDate', newValue)}
                                                    ...
                                                    <Box sx={{ p: 2, borderRadius: 1, mb: 3 }}>
                                                        ...
                                                        <RadioGroupList
                                                            ...
                                                            onChange={(e) => setFieldValue('deactivationReason', e.target.value)}
