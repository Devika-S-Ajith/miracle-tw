...
const Children = (props) => {
  ...
  const {organizationList, signedinUserRole, signedinOrgType} = useContext(CommonDataContext);
  ...
  const getChildren = useCallback(async (pageNumber = 1) => {
    setLoading(true)
    try {
  ...
  return (
    <Card {...props}>
      <CardHeader
        //action={<MoreMenu />}
        title={t('common:common.Children')}
      />
      <Divider />
        ...
        <Box sx={{ minWidth: 'auto' }}>
          <Table>
            <TableHead>
              <TableRow>
                <TableCell>
                {t('common:common.Child Name')}
                </TableCell>

                <TableCell>
                {t('common:common.Caregiver')}
                </TableCell>
            ...
            <TableBody>
              {children && children.length > 0 && children.map((child) => (
                <TableRow key={child && child.childId}>
                  ...
                  <TableCell>
                    {/* {familyMembers && familyMembers[Math.floor((Math.random()*familyMembers.length))]} */}
                    {child.familyMemberName}
                  </TableCell>

                  {/* <TableCell>
                  {`${care_givers.find(care_giver => child.care_giver_id === care_giver.care_giver_id).relation_with_child}`}
                  </TableCell> */}

                  <TableCell>
                  {` ${organizationList &&  organizationList.length &&
                   organizationList.find(item => item.id === child.HTOrganizationId).organizationName} `}     
                  </TableCell>

                  <TableCell>
                   {
                     child.userFirstName===null && child.userLastName===null ? "UnAssigned": `${child.userFirstName+ " " +child.userLastName}`
                   } 
                  ...
                  {((signedinOrgType == 3 || signedinOrgType == 4) && 
                    (signedinUserRole === 'admin' || signedinUserRole === 'caseworker') && child.HTOrganizationId === signedinOrgId)
                    ...
                    <Tooltip title={t('common:child.View Child')}>
                      <IconButton
                        component={RouterLink}
                        to={`/dashboard/child/${child && child.id}/view`}
                      >
