...
const Members = (props) => {
  ...
  const { familyMembers, familyId, pageCount, familyName } = props;
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>{t("common:common.Phone")}</TableCell>
                  <TableCell>{t("common:common.Occupation")}</TableCell>
                  <TableCell>{t("common:common.Email")}</TableCell>
                  <TableCell>{t("common:common.Member Type")}</TableCell>
                  ...
                  members.map((member) => (
                    ...
                    >
                      {care_giver.relation_with_child}
                        ...
                        {t(
                          `common:common.${member.HT_familyMemberType.memberType}`
                        )}
                      </TableCell>
                      <TableCell align="right">
                        <Tooltip title={t("common:common.Edit Caregiver")}>
                          <IconButton
                            ...
                            // to={`/dashboard/families/member/${member.id}/edit`}
                            id="edit-family"
                            onClick={() =>
                              navigate(
                                `/dashboard/families/member/${member.id}/edit`,
                                {
                                  state: {
                                    data: {
                        ...
                        <Tooltip title={t("common:common.View Caregiver")}>
                          <IconButton
                            //disabled={true}
                            // component={RouterLink}
                            // to={`/dashboard/families/member/${member.id}/view`}
                            id="view-family"
                            onClick={() =>
                              navigate(
                                `/dashboard/families/member/${member.id}/view`,
                                {
                                  state: {
                                    data: {
                                      familyName: familyName,
            ...
            <Box sx={{ width: "100%", ml: "40%", mt: 5, mb: 1 }}>
              <Box>
                <Grid container spacing={3}>
                  ...
                  >
                    <Typography>
                      {t("common:common.No Members to list")}
                    </Typography>
                  </Grid>
                </Grid>
              </Box>
