...
const AddAssessmentForm = (props) => {
  ...
  const getAssessmentDetails = useCallback(async (id, revision) => {
    ...
    try {
      const domainData = await APIS.DomainList();
      setDomains(domainData && domainData.data && domainData.data.data)
      console.log("assessmentDetailsdomainData", domainData.data.data, revision, id)
      //const HTLanguageId = currentLanguage === 'en' ? "" : getLanguageId()
      const data = await APIS.GetAssessmentDetails(id, revision);
      if (data && data.data) {
        console.log("assessmentDetailsAssessmentData", data.data)
  ...
  const getCaseDetails = useCallback(async (id) => {
    ...
    try {
      ...
      console.log("assessmentDetailsCaseData", data.data.data)
                      ...
                      <>
                        ...
                        >
                          <DatePicker
                            ...
                            value={dayjs(values.date_of_assessment)}
                            format={DateFormatFromRegion()}
                            maxDate={dayjs(values.date_of_assessment)}
                            disabled={true}
                            renderInput={(params) => (
