...
import { Box,Container, Grid, Typography,CircularProgress,Table,Card, MenuItem,Button,
   // TextField,
    ...
    TableCell,
    TableHead,
    TableRow, Pagination,FormControl,InputLabel, IconButton,TextField,Select } from '@material-ui/core';
...
function ReportsChildStatus() {
  ...
  const ClearFilters = () => {
   ...
   // setStartDate(null);
    ...
    useEffect(() => {
    document.title = "Reports | Child Status | Miracle Foundation"
    if(initialData===null)
    {
      getChildStatus();
    }
            ...
            <Table>
                ...
                <TableBody key={index}>
                    <TableRow >
                      ...
                      <TableCell>
                      {t(`common:common.${item.status}`)}
                      </TableCell>
                      <TableCell>
                      {item.currentPlacementStatus}
                      </TableCell>
                    </TableRow>      
                </TableBody>
                 )})}
