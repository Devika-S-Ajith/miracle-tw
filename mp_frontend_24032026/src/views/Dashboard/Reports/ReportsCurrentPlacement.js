...
function ReportsCurrentPlacement() {
  ...
  const tableBody = [{ key: "childId" }, { key: "currentPlacementStatus" }];
  ...
  useEffect(() => {
    document.title = "Reports | Current living condition | ThriveWell";
    if (initialData === null) {
      getNewlyAddedChildrenApi();
    } else if (initialData) {
      let payload = {
  ...
  const getNewlyAddedChildrenApi = useCallback(async (payload = null) => {
    try {
      ...
      finalPayload.isCurrentPlacementStatus = "true";
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      finalPayload.TWCountryId = localStorage.getItem("userRegion");
      const data = await APIS.CurrentPlacement(finalPayload);
      if (data?.data) {
        ConvertToXLSX(data?.data, "Children in Current living condition");
        setIsExportDisabled(false);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    ...
    >
      <ReportHeader reportHeaderText={t("common:common.Current Placement")} />
      {/* <ReportExportButton
...
export default ReportsCurrentPlacement;
