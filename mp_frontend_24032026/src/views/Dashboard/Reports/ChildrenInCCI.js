...
function ChildrenInCCI() {
      ...
      render: (item) =>
        ...
        `${
          childPlacementList.find(
            (item1) => item1.id === item.HTChildPlacementStatusId
          )?.placementStatus || "--"
        }`,
    },
    {
  ...
  useEffect(() => {
    document.title = "Reports | Children in CCI | ThriveWell";
    if (initialData === null) {
  ...
  return (
    ...
    >
      <ReportHeader reportHeaderText={t("common:common.Children in CCI")} />
      {/* <ReportExportButton
